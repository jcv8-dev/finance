...
function monatAusgabenOhneAnlagen($date){
    $monat = $date->format("m");
    $jahr = $date->format("Y");
    $sql = "select SUM(betrag) from buchungen join kategorie on buchungen.kategorieid = kategorie.id where kategorie.kategorieBezeichnung is not 'Anlage' and MONTH(datum) = $monat and YEAR(datum) = $jahr;";
    $result = db()->query($sql);
    return $result;
}
